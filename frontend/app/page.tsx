...
export default function Home() {
  ...
  const wakeLockLabel = !wakeLockSupported
    ? "Wake Lock yok"
    : wakeLockActive
      ? "Ekran açık"
      : "Ekranı açık tut";
  ...
  return (
    <main className="relative min-h-screen overflow-hidden bg-black px-4 py-4 text-[var(--ivory)] sm:px-6">
      <div
        aria-hidden="true"
      ...
      <div
        className={`relative mx-auto grid min-h-[calc(100vh-2rem)] w-full max-w-6xl gap-4 ${
      ...
      >
            ...
            <div className="mb-3 flex items-center justify-between gap-3 text-[10px] uppercase tracking-[0.3em] text-[var(--ivory-muted)] sm:text-[11px]">
              ...
              <div className="flex items-center gap-2">
                ...
                <button
                  ...
                  className="min-h-[60px] rounded-full border border-white/12 bg-white/5 px-4 text-[10px] uppercase tracking-[0.22em] text-[var(--ivory)] transition active:scale-[0.98] disabled:cursor-not-allowed disabled:opacity-50 sm:text-[11px]"
        ...
        <section className="flex flex-col gap-4 rounded-[2rem] border border-white/8 bg-white/[0.03] p-4 shadow-[0_0_50px_rgba(241,235,220,0.03)] backdrop-blur-[1px]">
          <div className="grid grid-cols-3 gap-2">
            {[
              { id: "surus" as const, label: "SÜRÜŞ" },
              { id: "motor" as const, label: "MOTOR" },
              { id: "saglik" as const, label: "SAĞLIK" },
            ].map((tab) => {
